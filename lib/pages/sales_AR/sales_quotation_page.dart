 ...
 Widget _buildTabSection() {
  return Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border.all(
        color: Colors.white, // GANTI: Pakai Indigo agar garis birunya tebal melingkar
        width: 3.5, 
      ),
      boxShadow: [
        BoxShadow(
          ...
          child: Scrollbar(
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                border: TableBorder(
                  verticalInside: BorderSide(color: const Color.fromARGB(208, 166, 164, 164), width: 0.5),
                  horizontalInside: BorderSide(color: const Color.fromARGB(208, 166, 164, 164), width: 0.5),
                ),
...
List<DataColumn> _buildStaticColumns() {
 ...
 Widget _buildAccountingTab() {
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Expanded(child: _buildModernCheckbox("Use Shipped Goods Account", "cb_shipped_acc")),
                ],
              ),
            ],
          ),
  ...
  return Container(
    ...
    margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
    padding: const EdgeInsets.all(24), // Ruang napas internal agar konten tidak mepet
    clipBehavior: Clip.antiAlias,
    decoration: BoxDecoration(
      color: Colors.white,
...
Widget _buildModernFooter() {
 ...
 Widget _buildActionButtons() {
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Container(
            height: 28, decoration: BoxDecoration(color: isReadOnly ? bgSlate : Colors.white, border: Border.all(color: borderGrey), borderRadius: BorderRadius.circular(4)),
            child: TextField(
              controller: controller, readOnly: isReadOnly, textAlign: TextAlign.right, style: TextStyle(fontSize: 12, fontWeight: isBold ? FontWeight.bold : FontWeight.w500),
  ...
  Widget _buildSummaryBox(String key, {String defaultValue = "0.00", bool isReadOnly = false, bool isPercent = false}) {
    final controller = _getCtrl(key, initial: _fieldValues[key] ?? defaultValue);
    return Container(
      height: 24, decoration: BoxDecoration(color: isReadOnly ? bgSlate : Colors.white, border: Border.all(color: borderGrey), borderRadius: BorderRadius.circular(4)),
      child: TextField(
  ...
  return Padding(
    ...
    padding: EdgeInsets.zero, 
    child: Row(
      crossAxisAlignment: CrossAxisAlignment.center, // Sejajarkan label dengan input
      children: [
        SizedBox(
  ...
  Widget _buildSmallDropdown(String key, List<String> items) {
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 8), height: 30, decoration: BoxDecoration(color: Colors.white, border: Border.all(color: borderGrey), borderRadius: BorderRadius.circular(6)),
 ...
 Widget _buildSmallDropdownRowModern(String label, String key, List<String> items) {
  ...
  Widget _buildChooseFromListField(String label, String key, List<String> data) {
    return Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [
      ...
      Expanded(child: InkWell(
        ...
        child: Container(height: 30, decoration: BoxDecoration(color: bgSlate, border: Border.all(color: borderGrey), borderRadius: BorderRadius.circular(6)),
          child: Row(children: [
            Expanded(child: Padding(padding: const EdgeInsets.only(left: 8), child: Text(_getCtrl(key).text.isEmpty ? data.first : _getCtrl(key).text, style: const TextStyle(fontSize: 12)))),
  ...
  Widget _buildFileUploadRow(String label, String key) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(children: [
      ...
      Expanded(child: InkWell(
        ...
        child: Container(height: 28, decoration: BoxDecoration(border: Border.all(color: borderGrey), borderRadius: BorderRadius.circular(4)),
          child: Row(children: [
            Expanded(child: Padding(padding: const EdgeInsets.only(left: 8), child: Text(_formValues[key] ?? "No file selected", style: const TextStyle(fontSize: 11)))),
  ...
  Widget _buildFloatingSidePanel() {
    return Container(width: 380, decoration: const BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 10, offset: Offset(-2, 0))]),
      child: Column(children: [
        AppBar(backgroundColor: primaryIndigo, title: const Text("Sales Order", style: TextStyle(fontSize: 14)), actions: [IconButton(onPressed: () => setState(() => showSidePanel = false), icon: const Icon(Icons.close))]),
        Expanded(child: ListView(padding: const EdgeInsets.all(16), children: [
         _buildChooseFromListField("Business Unit", "cfg_bu", [""]),
          ...
          const SizedBox(height: 20),
          ElevatedButton(onPressed: () => setState(() => showSidePanel = false), style: ElevatedButton.styleFrom(backgroundColor: Colors.green), child: const Text("APPLY", style: TextStyle(color: Colors.white))),
        ])),
      ]),
    );
  }
